...
from mlx.jira_juggler.utils.sprint import Sprint, SprintState
...
"""
    ...
    def factory(cls, registry, to_username, jira_issue, parent=None) -> "JugglerTask":
        if jira_issue.fields.issuetype.name == cls.TYPE.INITIATIVE:
            ...
            jira_issue.fields.issuetype.name == cls.TYPE.SPIKE
            or "[spike]" in jira_issue.fields.summary.lower()
        ):
            return Spike(registry, to_username, jira_issue, parent)
        elif (
            jira_issue.fields.issuetype.name == cls.TYPE.SUBTASK
            and "[qa auto]" in jira_issue.fields.summary.lower()
        ):
            return QaAutoSubtask(registry, to_username, jira_issue, parent)
        elif (
            jira_issue.fields.issuetype.name == cls.TYPE.SUBTASK
            and "[qa manual]" in jira_issue.fields.summary.lower()
        ):
            return QaManualSubtask(registry, to_username, jira_issue, parent)
        elif (
            jira_issue.fields.issuetype.name == cls.TYPE.SUBTASK
            and re.match(r"\[[^\[\]]*bug\].+", jira_issue.fields.summary.lower())
            != None
        ):
    ...
    def load_from_jira_issue(self, jira_issue):
        ...
        self.summary = (
            (summary[: self.MAX_SUMMARY_LENGTH] + "...")
        ...
        self.children = [
            JugglerTask.factory(self.registry, self.to_username, child, self)
            for child in jira_issue.children
        ]
        if len(self.children) > 0:
            self.properties["effort"].update(
                CompositePertEstimate(
                    [child.properties["effort"].pert for child in self.children]
                )
            )
    ...
    def determine_resolved_at_date(self):
        ...
        for change in sorted(
            self.issue.changelog.histories, key=attrgetter("created"), reverse=True
        ):
            for item in change.items:
                if item.field.lower() == "status":
                    ...
                    if status in RESOLVED_STATUSES:
                        ...
                        status in DONE_STATUSES + PENDING_STATUSES
                        and closed_at_date is None
                    ):
                        closed_at_date = parser.isoparse(change.created)
        return closed_at_date

    def shift_unstarted_tasks_to_milestone(self, milestone: Sprint):
        ...
        #     self.properties["fact:depends"].append_value(sprint.name)
            ...
            for child in self.children:
                child.shift_unstarted_tasks_to_milestone(sprint)
        """
        self.properties['fact:depends'].append_value(sprint.name)
    ...
    def starts_after_sprint_start(self, sprint: Sprint) -> bool:
        if self.children:
            return all(
                child.starts_after_sprint_start(sprint) for child in self.children
            )
        if self.properties["time"].is_empty:
            return True
        if self.properties["time"].name in (
            "start",
            "fact:start",
        ):
            return self.properties["time"].value >= sprint.start
        if self.properties["time"].name in ("fact:end",):
            ...
            return start_time >= sprint.start
        return False
                    ...
                    if dt is not None:
                        if not self.properties["time"].is_empty:
                            if "start" in self.properties["time"].name:
                                ...
                                end_time = self.properties["time"].value
                                days_spent = (
                                    self.properties["effort"].value / self.focus_factor
                                )
                                start_time = self.add_working_days(
                                    end_time, -days_spent
                                )
                            if dt > start_time:
                                logging.warning(
